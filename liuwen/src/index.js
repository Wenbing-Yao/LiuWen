...
const { isDev, dirConfig, domain, URLS } = require("./modules/config");
...
function getPeClient(username = null, onLine = true) {
    if (username == null) username = settingStorage.getUsername();
...
const createWindow = async () => {
    ...
    let icon_fpath = null;
    if (isMac) {
        icon_fpath = path.join(__dirname, "static/icons/icon.icns");
    } else if (isWin) {
        icon_fpath = path.join(__dirname, "static/icons/icon.ico");
    } else if (isLinux) {
        icon_fpath = path.join(__dirname, "static/icons/icon.ico");
    } else {
        icon_fpath = path.join(__dirname, "static/icons/icon.png");
    }

    let icon_image = nativeImage.createFromPath(icon_fpath);
    ...
    if (!mainWindow) {
        ...
        if (isMac) {
            app.dock.setIcon(icon_image);
        } else if (isWin) {
            mainWindow.setIcon(icon_image);
        } else if (isLinux) {
            mainWindow.setIcon(icon_image);
        } else {
            mainWindow.setIcon(icon_image);
    ...
    if (isMac) {
        ...
        menuTemplate = require("./menus/linux").menuTemplate;
    } else {
    ...
    if (!isMac)
        app.setAboutPanelOptions({
            ...
            iconPath: path.join(__dirname, "static/icons/icon.png"),
        });
    ...
    // view.setBounds({ x: 20, y: 20, width: 600, height: 800 })
...
ipcMain.on("article:create", (event, mdContent, articleInfo) => {
    ...
    if (!filePath) {
        event.reply("article:create-reply", filePath);
        return;
    }
...
ipcMain.on("article:preview", (event, localId) => {
    const { openArticlePreviewModal } = require("./modules/Modal");
    openArticlePreviewModal(localId, mainWindow);
});
...
ipcMain.on("article:modal-preview-close", (event, localId) => {
    closeArticlePreviewModal(localId);
});

ipcMain.on(
    "article:modal-delete-confirm",
...
function buildLocaleLogin(callback) {
    ...
    env.render(
        index_fpath,
        {
            ...
            register_url: `${domain}/${URLS.register}`,
        },
        (err, res) => {
            if (err) {
                log.error(err);
...
ipcMain.on("link:open", (event, link) => {
    ...
    log.info(`open external link ${link}`);
    shell.openExternal(link);
});

ipcMain.on("localfile:open", (event, fpath) => {
    if (fpath.startsWith("file://")) {
        fpath = fpath.slice(7);
    }

    try {
        let info = `open local file ${fpath}`;
        if (isDev) {
            console.log(info);
        }
        log.info(info);
        ...
        let info = `local file does not exist: ${fpath}`;
        if (isDev) {
            console.log(info);
        }
        log.info(info);
